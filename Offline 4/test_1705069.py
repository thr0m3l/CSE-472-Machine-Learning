...
if __name__ == '__main__':
    ...
    test_X = sorted(glob.glob(os.path.join(data_dir, 'training-d', '*.png')))
    ...
    X_test_all, y_test_all = get_data(test_X, test_label, resize_dim=28)
    X_test_all = X_test_all.reshape(X_test_all.shape[0], 28, 28, 1)

    # Convert from one hot encoding to single number
